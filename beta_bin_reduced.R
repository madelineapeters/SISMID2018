...
#Goals: update thetas (Gibbs steps), two MH steps for alpha and beta
...
#Codes posterior distribution on log scale, omitting constants
log_posterior = function(my_alpha, my_beta, theta_vec, my_data, sample_size, prior_inten_alpha, prior_inten_beta){
  #lgamma returns natural log of the absolute value of the gamma function

  #gamma(alpha+beta)/gamma(alpha)*gamma(beta) -> lgamma(alpha+beta) - lgamma(alpha) - lgamma(beta)
    # + log(theta_vec)*(my_data$x+alpha-1) + log(1-theta_vec)sum(my_data$n-my_data$x+beta-1+)
    # - prior_inten_alpha*alpha - prior_inten_beta*beta
    # originally taking products across i, but since log now add (sum)
...
#Take my_data (matrix 72x2, # w/ cancer and total colony size (xi and ni))
#Arbitrary alpha and beta values
#prior_inten_alpha and beta assumptions that alpha and beta apriori are independent and come from exponential distirbution with rate 0.1
#tuning parameters for alpha and beta
#mcmc_burnin are how many samples to not record
#mcmc_subsample
...
mcmc_sampler = function(my_data, init_alpha, init_beta, prior_inten_alpha, prior_inten_beta,
  ...
  theta.update = function(c,rat_data,cur_alpha,cur_beta){
    out = rbeta(1,rat_data[c,1]+cur_alpha,rat_data[c,2]-rat_data[c,1]+cur_beta)
    return(out)
  }
  ...
  for (i in 1:mcmc_size){
    ...
    cur_theta = sapply(1:data_sample_size,theta.update,rat_data,cur_alpha,cur_beta)
    
    ## Metropolis step for alpha
    ## TO DO: IMPLEMENT M-H STEP HERE FOR ALPHA
    new_alpha = propose_pospar(cur_alpha, runif(1), tuning_alpha)
    
    log_acc_alpha = log_alpha_posterior(new_alpha, cur_beta, cur_theta, my_data, data_sample_size, prior_inten_alpha) -
